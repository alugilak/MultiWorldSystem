...
public class armorFeet implements CommandExecutor {	
...
public armorFeet( leerWelt instance)
{
...
public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) { 
	Player p = (Player)sender;		  	    
	      if (commandLabel.equalsIgnoreCase("armorfeets")) {
	    	  if (!sender.hasPermission((String)this.settings.getpermData().get("mwsenchant")))
	        {
   		  ...
   		  p.sendMessage(ChatColor.GOLD.toString() + ChatColor.BOLD + ((String)this.settings.getsysoData().get("SystemName")) + ChatColor.GOLD.toString() + ChatColor.BOLD + " >" + ChatColor.RED + "Use /armorfeets <value> <Lore> <Displayname>");
   		  	return false;}}
   		  if (args.length == 3);{
