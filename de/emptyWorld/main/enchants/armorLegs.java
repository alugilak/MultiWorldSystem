...
public class armorLegs implements CommandExecutor {	
...
public armorLegs( leerWelt instance)
{
...
public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) { 
	Player p = (Player)sender;		  	    
	      if (commandLabel.equalsIgnoreCase("armorlegs")) {
	    	  if (!sender.hasPermission((String)this.settings.getpermData().get("mwsenchant")))
	        {
	    	  ...
	    	  StringBuilder str = new StringBuilder();
	   		  ...
	   		  p.sendMessage(ChatColor.GOLD.toString() + ChatColor.BOLD + ((String)this.settings.getsysoData().get("SystemName")) + ChatColor.GOLD.toString() + ChatColor.BOLD + " >" + ChatColor.RED + "Use /armorlegs <value> <Lore> <Displayname>");
	   		  	return false;}}
	   		  if (args.length == 3);{
