...
public class jump implements CommandExecutor
{
  ...
  if ((cmd.getName().equalsIgnoreCase("jump"))){
	  if (!sender.hasPermission((String)this.settings.getpermData().get("mwsPoitions"))) {		
	        ...
	        return false;
              ...
              sender.sendMessage(ChatColor.RED + "Too few arguments! make /jump <value> and or <playername> ");}else if(args.length < 2){      
	      int nv = Integer.parseInt(args[0]);
      PotionEffect effect = new PotionEffect(org.bukkit.potion.PotionEffectType.JUMP, Integer.MAX_VALUE, nv, true, false);
  ...
  PotionEffect effect1 = new PotionEffect(org.bukkit.potion.PotionEffectType.JUMP, Integer.MAX_VALUE, nv, true, false);
