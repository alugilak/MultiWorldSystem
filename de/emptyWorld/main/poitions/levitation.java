...
public class levitation implements CommandExecutor
{
  ...
  if ((cmd.getName().equalsIgnoreCase("levitation"))){
	  if (!sender.hasPermission((String)this.settings.getpermData().get("mwsPoitions"))) {		
	        ...
	        return false;
              ...
              sender.sendMessage(ChatColor.RED + "Too few arguments! make /levitation <value> and or <playername> ");}else if(args.length < 2){      
	      int nv = Integer.parseInt(args[0]);
      PotionEffect effect = new PotionEffect(org.bukkit.potion.PotionEffectType.LEVITATION, Integer.MAX_VALUE, nv, true, false);
  ...
  PotionEffect effect1 = new PotionEffect(org.bukkit.potion.PotionEffectType.LEVITATION, Integer.MAX_VALUE, nv, true, false);
