...
public class confusion implements CommandExecutor
{
	einstellungen settings = einstellungen.getInstance();
	  ...
	  leerWelt plugin;   
  public confusion( leerWelt instance)
  {
  ...
  if ((cmd.getName().equalsIgnoreCase("confusion"))){
      ...
      PotionEffect effect = new PotionEffect(org.bukkit.potion.PotionEffectType.CONFUSION, Integer.MAX_VALUE, nv, true, false);
      ((Player)sender).addPotionEffect(effect);
      p.sendMessage(ChatColor.GREEN + ((String)this.settings.getsysoData().get("mwsconfusion")));
      }else{   if (args.length < 3);{
	  int nv = Integer.parseInt(args[0]);
	  Player player = Bukkit.getPlayer(args[1]);
  PotionEffect effect1 = new PotionEffect(org.bukkit.potion.PotionEffectType.CONFUSION, Integer.MAX_VALUE, nv, true, false);
